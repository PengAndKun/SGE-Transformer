...
class Visu():
    ...
    def visu_path(self, path, Hori_ActionTransitionMatrix=None):
        f, ax = plt.subplots(figsize=(self.x_shape, self.y_shape))
        # path = [(0, 11), (1, 15), (10_e, 14), (3, 13), (4, 9)]
        x = []
        y = []
        ...
        for cell in range(self.x_shape*self.y_shape):
            ...
            if Hori_ActionTransitionMatrix is not None:
                is_obstacle = True
                for action in range(self.action_dim):
                    if Hori_ActionTransitionMatrix[cell, cell, action] != 1.0:
                        is_obstacle = False
                        break
                if is_obstacle:
                    list_traj_subgrad.append("X")  # Unreachable state
                else:
                    list_traj_subgrad.append("") # Reachable state
            else:
