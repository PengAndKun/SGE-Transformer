...
# 随机分支扩展逻辑
# ==========================================
def expand_node_random(env, parent_node, action_space, branching_factor):
    """
    ...
    return candidates[:],computing_count
...
def run_hybrid_session(num_batches, total_budget, seed, run_id):
    ...
    for batch_idx in range(config.NUM_BATCHES):
        ...
        while batch_steps < STEPS_PER_BATCH:
            ...
            top_k_results,computing_count = expand_node_random(env, parent_node, action_space, config.BRANCHING_FACTOR)
...
if __name__ == "__main__":
    ...
    for j in BATCHES:
        for budget in BUDGETS:
            SAVE_FILE = f"../../data_pybullet/3d_trajectories/hybrid_random/hybrid_random_trajectories_batches_{j}_budget_{budget}.pkl"
            ...
            if len(all_valid_data) > 0:
                ...
                print(f"SUCCESS: Saved {len(kept_trajs)}  random trajectories to {SAVE_FILE}")
            else:
