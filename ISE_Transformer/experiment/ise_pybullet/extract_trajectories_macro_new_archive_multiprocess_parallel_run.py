...
class CellNode:
    __slots__ = ['node_id', 'parent_id', 'snapshot', 'cumulative_reward',
                 'global_step', 'times_selected', 'action_from_parent',
                 'last_state', 'last_reward']

    def __init__(self, node_id, parent_id, snapshot, cumulative_reward, global_step,
                 action_from_parent=None, last_state=None, last_reward=0.0):
        self.node_id = node_id
        self.parent_id = parent_id # 存储父节点的唯一 ID，而非 Key
...
def reconstruct_history(node_id, archive):
    """通过 node_id 链条完美重建轨迹，不会出现覆盖导致的断裂"""
    states, rewards, actions = [], [], []
    curr_id = node_id
    while curr_id is not None:
        node = archive[curr_id]
        states.append(node.last_state)
        if node.action_from_parent is not None:
            actions.append(node.action_from_parent)
            rewards.append(node.last_reward)
        curr_id = node.parent_id
    return {
        "states": np.array(states[::-1], dtype=np.float32),
        "rewards": np.array(rewards[::-1], dtype=np.float32),
        "actions": np.array(actions[::-1], dtype=np.int32)
    }
...
def run_extraction_session(num_batches, total_budget, seed, run_id, score_threshold=99.0):
    ...
    with suppress_output():
        env = CoverageAviary(gui=True, obstacles=True, ctrl_freq=30,num_rays=120,
    ...
    root_state = np.concatenate([env.pos[0].copy(), env.rpy[0].copy()])
    root_node = CellNode(node_id=node_counter, parent_id=None,
                         snapshot=env.get_snapshot(), cumulative_reward=0.0,
                         global_step=0, last_state=root_state)

    global_archive[node_counter] = root_node
    root_grid_key = get_cell_key(env.pos[0], 0, config)
    grid_to_node_id[root_grid_key] = node_counter
    node_counter += 1
    ...
    for batch_idx in range(config.NUM_BATCHES):
        ...
        while steps_count < STEPS_LIMIT_PER_BATCH:
            ...
            env.restore_snapshot(curr_node.snapshot)
        ...
        # F. 释放非 Elite 叶子节点的 Snapshot
        elite_ids = {n.node_id for n in elites}
        for nid, node in global_archive.items():
            if nid not in elite_ids and nid != 0:  # 保留精英和根节点的快照
                # 注意：如果为了更稳健，可以像之前说的保留所有 keep_ids 的快照
                # 这里只保留叶子节点的快照以压榨内存
                node.snapshot = None
...
def run_parallel_extraction():
    ...
    BATCHES = 10
    BUDGET = 50000
    NUM_PROCESSES = 1       # [功能增加] 指定同时运行的进程数

    REPEATS = 20  # [修改] 跑 30 次
    SCORE_THRESHOLD = 0.0  # [修改] 分数阈值
    PERCENTILE_KEEP = 0  # 可调：90 表示保留 ≥90分位（前10%）
    TOP_K_TRAJS = 500
    SAVE_FILE = f"elite_trajectories_v1_multiprocessing_{BATCHES}_{BUDGET}.pkl"
