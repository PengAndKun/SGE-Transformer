...
class RNNBCNetwork(nn.Module):
    ...
    def forward(self, state_sequences):
        ...
        rnn_out, _ = self.rnn(state_sequences)
...
class RNNBCTrainer:
    def __init__(self, network: RNNBCNetwork, lr: float = 1e-4, device: str = None):
    ...
    def learn(self, state_sequences, action_sequences, batch_size, num_epochs):
        ...
        epoch_pbar = tqdm(range(num_epochs), desc="RNN BC Training", unit="epoch")
        ...
        for epoch in epoch_pbar:
            ...
            if (epoch ) % 500 == 0:
                self.save_model(f"sge_rnn_bc_model_{epoch}.pth")
...
if __name__ == "__main__":
    ...
    print("Initializing RNN BC Network...")
    network = RNNBCNetwork(STATE_DIM, NUM_ACTIONS, HIDDEN_DIM, NUM_LAYERS)
    trainer = RNNBCTrainer(network, lr=1e-4, device=DEVICE)

    if os.path.exists(PKL_FILE):
        trainer.train_from_pkl(PKL_FILE, H, BATCH_SIZE, EPOCHS)
        trainer.save_model(f"sge_rnn_bc_model_{EPOCHS}.pth")
    else:
