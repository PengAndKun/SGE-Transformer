...
class TrajectoryTransformer(nn.Module):
    def __init__(
        ...
        # Reward Embedding (Single step reward, NOT return-to-go)
        ...
        # --- Prediction Heads ---
        # 1. Action Head: Predict a_t given s_t
        self.predict_action = nn.Sequential(
            nn.Linear(hidden_size, hidden_size), 
            nn.ReLU(), 
            nn.Linear(hidden_size, act_dim)
        )
        # 2. Reward Head: Predict r_t given s_t, a_t
        self.predict_reward = nn.Sequential(
            nn.Linear(hidden_size, hidden_size), 
            nn.ReLU(), 
            nn.Linear(hidden_size, 1)
        )
        # 3. State Head (Dynamics): Predict s_{t+1} given s_t, a_t, r_t
    ...
    def forward(self, states, actions, rewards, timesteps):
        """
        Input Sequence Order: (s_1, a_1, r_1, s_2, a_2, r_2, ...)
        ...
        # 1. Action Prediction: Input s_t (idx 0, 3, 6...) -> Predict a_t
        action_feats = hidden_states[:, 0::3, :] 
        action_preds = self.predict_action(action_feats)  # (batch, seq_len, act_dim)

        # 2. Reward Prediction: Input a_t (context involves s_t, a_t, idx 1, 4, 7...) -> Predict r_t
        reward_feats = hidden_states[:, 1::3, :]
        reward_preds = self.predict_reward(reward_feats) # (batch, seq_len, 1)

        # 3. State Prediction: Input r_t (context involves s_t, a_t, r_t, idx 2, 5, 8...) -> Predict s_{t+1}
        state_feats = hidden_states[:, 2::3, :]
        state_preds = self.predict_state(state_feats)    # (batch, seq_len, state_dim)
...
class TTTrainer:
    def __init__(self, model, lr=1e-4, device='cpu'):
        ...
        # Loss Functions
        self.loss_cls = nn.CrossEntropyLoss(reduction='none') # For Discrete Actions
        self.loss_state = nn.MSELoss(reduction='none')        # For Continuous State
        self.loss_reward = nn.MSELoss(reduction='none')       # For Continuous Reward
    ...
    def get_batch(self, batch_size, normalize_params):
        ...
        for _ in range(batch_size):
            ...
            # Next States Labels
            # Assuming state_sequence is [s0, s1, s2...] and action_sequence is [a0, a1, a2...]
            # s[t] corresponds to a[t]. The next state is s[t+1].
            full_s = traj['state_sequence']
            
            # Indices for next states
            ns_indices = np.arange(s_start + 1, si + 2) # shifted by 1
            # Handle end of episode: clamp to last state
    ...
    def train(self, steps, batch_size, normalize_params):
        self.model.train()
        pbar = tqdm(range(steps), desc="TT Training")
        ...
        for step in pbar:
            ...
            # 2. Reward Loss
            loss_r = self.loss_reward(r_preds.reshape(-1, 1), rewards.reshape(-1, 1))
            loss_r = (loss_r.squeeze() * mask).sum() / denom
            
            # 3. State Loss (Dynamics)
            loss_s = self.loss_state(s_preds.reshape(-1, self.model.state_dim), next_states.reshape(-1, self.model.state_dim))
            loss_s = (loss_s.mean(dim=1) * mask).sum() / denom
            
            # Weighted Sum (Simple 1:1:1 weighting for now)
            ...
            if (step) % 500 == 0:
                 torch.save(self.model.state_dict(), f"sge_tt_model_{step}.pth")
...
if __name__ == "__main__":
    ...
    candidates = [
        PKL_FILE,
        os.path.join(current_dir, PKL_FILE),
        os.path.join(os.path.dirname(current_dir), PKL_FILE),
        os.path.join("SGE_Transformer/experiment/sge_pybullet", PKL_FILE)
    ]
    ...
    trainer = TTTrainer(tt_model, device="cuda" if torch.cuda.is_available() else "cpu")
    ...
    torch.save(tt_model.state_dict(), f"sge_tt_model_{STEPS}.pth")
    print(f"Model saved: sge_tt_model_{STEPS}.pth")
