...
from SGE_Transformer.experiment.sge_pybullet.sge_cql import CQLNetwork

def test_cql(model_path, num_test_episodes=10, stochastic=True):
    STATE_DIM = 6
    NUM_ACTIONS = 27
    HIDDEN_DIM = 256
    MOVE_DISTANCE = 0.5
    ...
    print(f"Loading CQL model from: {model_path}")
    network = CQLNetwork(STATE_DIM, NUM_ACTIONS, HIDDEN_DIM).to(DEVICE)
    ...
    for episode in range(num_test_episodes):
        ...
        while not done and step_count < max_steps:
            ...
            with torch.no_grad():
                # stochastic decides if we sample from softmax(Q) or argmax(Q)
...
if __name__ == "__main__":
    ...
    EPOCHS = 20000
    INTERVAL = 500
    RESULTS_FILE = "cql_benchmark_results.csv"

    all_results = []

    for epoch in range(0, EPOCHS + 1, INTERVAL):
        MODEL_NAME = f"sge_cql_model_{epoch}.pth"
        PATHS_TO_CHECK = [
            MODEL_NAME,
        ...
        if MODEL_PATH:
            ...
            avg_rew, std_rew, avg_cov, std_cov = test_cql(MODEL_PATH, num_test_episodes=20, stochastic=True)
