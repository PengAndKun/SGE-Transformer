...
# 导入我们刚刚定义的 DecisionTransformerGPTNeo
from SGE_Transformer.experiment.sge_pybullet.sge_decision_transformer_gptneo import DecisionTransformerGPTNeo

def test_decision_transformer_gptneo(model_path, num_test_episodes=10, target_return=1000.0, stochastic=True):
    # --- 1. 参数配置 (需与训练一致) ---
    STATE_DIM = 6
    ACT_DIM = 27
    CONTEXT_LEN = 20
    ...
    print(f"Strategy: {'Stochastic (Sampling)' if stochastic else 'Deterministic (Greedy)'}")

    # --- 2. 加载模型 ---
    print(f"Loading DT-GPTNeo model from {model_path}...")
    
    model = DecisionTransformerGPTNeo(
        state_dim=STATE_DIM,
        act_dim=ACT_DIM,
    ...
    if os.path.exists(model_path):
        ...
        if isinstance(checkpoint, dict) and 'state_dict' in checkpoint:
            model.load_state_dict(checkpoint['state_dict']) # 未知情况
        elif isinstance(checkpoint, dict):
             # 我们的保存方式通常是直接 save state_dict
    ...
    for episode in range(num_test_episodes):
        ...
        print(f"\n--- Episode {episode + 1} Start (Target: {target_return}) ---")
        ...
        while not done and step_count < max_steps:
            ...
            if step_count % 10 == 0:
                print(f"Step {step_count} | Action: {action_id} | Reward: {step_reward:.2f} | RTG: {current_rtg:.2f}")
...
if __name__ == "__main__":
    ...
    EPOCHS = 20000
    INTERVAL = 500
    RESULTS_FILE = "sge_dt_gptneo_benchmark_results.csv"

    all_results = []
        
    for epoch in range(0, EPOCHS + 1, INTERVAL):
        MODEL_NAME = f"sge_dt_gptneo_model_{epoch}.pth"
        MODEL_PATH = None
        candidates = [
            MODEL_NAME,
        ...
        if MODEL_PATH:
            ...
            avg_r, std_r, avg_c, std_c = test_decision_transformer_gptneo(MODEL_PATH, num_test_episodes=20, target_return=1000.0, stochastic=True)
