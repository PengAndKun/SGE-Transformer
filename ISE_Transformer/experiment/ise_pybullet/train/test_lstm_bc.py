...
from SGE_Transformer.experiment.sge_pybullet.sge_lstm_bc import LSTMBCNetwork

def test_lstm_bc(model_path, num_test_episodes=10, stochastic=True):
    # --- 1. Config ---
    STATE_DIM = 6
    NUM_ACTIONS = 27
    H = 20  
    ...
    print("\n" + "="*40)
    print(f"📊 LSTM Test Summary ({num_test_episodes} Episodes)")
...
if __name__ == "__main__":
    ...
    EPOCHS = 20000
    INTERVAL = 500
    RESULTS_FILE = "lstm_bc_benchmark_results.csv"

    all_results = []

    for epoch in range(0, EPOCHS + 1, INTERVAL):
        MODEL_NAME = f"sge_lstm_bc_model_{epoch}.pth"
        PATHS_TO_CHECK = [
            MODEL_NAME,
        ...
        if MODEL_PATH:
            ...
            avg_rew, std_rew, avg_cov, std_cov = test_lstm_bc(MODEL_PATH, num_test_episodes=20, stochastic=True)
