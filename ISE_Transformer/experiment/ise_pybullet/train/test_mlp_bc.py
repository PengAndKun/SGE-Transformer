...
from SGE_Transformer.experiment.sge_pybullet.sge_mlp_bc import MLPBCNetwork

def test_mlp_bc(model_path, num_test_episodes=10, stochastic=True):
    STATE_DIM = 6
    NUM_ACTIONS = 27
    HIDDEN_DIM = 256
    MOVE_DISTANCE = 0.5
    ...
    print(f"Loading MLP BC model from: {model_path}")
    network = MLPBCNetwork(STATE_DIM, NUM_ACTIONS, HIDDEN_DIM).to(DEVICE)
    ...
    for episode in range(num_test_episodes):
        ...
        while not done and step_count < max_steps:
            # Prepare Input (No sequence, just current state)
...
if __name__ == "__main__":
    ...
    EPOCHS = 20000
    INTERVAL = 500
    RESULTS_FILE = "mlp_bc_benchmark_results.csv"

    all_results = []

    for epoch in range(0, EPOCHS + 1, INTERVAL):
        MODEL_NAME = f"sge_mlp_bc_model_{epoch}.pth"
        ...
        if MODEL_PATH:
            ...
            avg_rew, std_rew, avg_cov, std_cov = test_mlp_bc(MODEL_PATH, num_test_episodes=20, stochastic=True)
