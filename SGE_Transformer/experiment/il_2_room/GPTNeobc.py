...
class HuggingFaceGptNeoBCNetwork(nn.Module):
    def __init__(self, state_dim: int, num_actions: int = 9, d_model: int = 128, num_decoder_layers: int = 3, nhead: int = 8, max_seq_length: int = 200):
        ...
        config = GPTNeoConfig(vocab_size=1,attention_types=[[["global"], 2], [["local"], 1]], max_position_embeddings=max_seq_length, hidden_size=d_model, num_layers=num_decoder_layers, num_heads=nhead)
...
#     state_dim = 198
#     hf_transformer_network = HuggingFaceTransformerBCNetwork(
#         state_dim=state_dim, num_actions=4, d_model=128,
#         num_decoder_layers=3, nhead=8, max_seq_length=80
#     )
