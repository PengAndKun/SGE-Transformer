...
# 引入依赖
from SGE_Transformer.envs.coverage_lidar_aviary_with_obs20 import CoverageAviary
from stable_controller import ROS2VelocityController

# ==========================================
# [New] Constraint Configuration
# ==========================================
CEILING_HEIGHT = 2.5  # Max height in meters
...
class CellNode:
    def __init__(self, key, snapshot, cumulative_reward, global_step, pos, rpy, obs, parent_key=None,
...
def reconstruct_full_trajectory(node: CellNode, archive: Dict[tuple, CellNode]):
    ...
    while current_node.parent_key is not None:
        ...
        if current_node.parent_key in archive:
            ...
            curr_state = list(current_node.obs)
...
def run_extraction_session(num_batches, total_budget, seed, run_id, score_threshold=99.0):
    ...
    with suppress_output():
        env = CoverageAviary(gui=False, obstacles=True, user_debug_gui=False)
    ...
    root_node = CellNode(start_key, start_snapshot, 0.0, global_step=0, pos=start_pos, rpy=start_rpy,obs=obs[0])
    ...
    for batch_idx in range(config.NUM_BATCHES):
        ...
        while current_batch_steps_counter < STEPS_LIMIT_PER_BATCH:
            ...
            # print(f' env',current_node.pos," --  ",current_node.key)
            # print(f' env pre ',env.pos)
            env.restore_snapshot(current_node.snapshot)
            # print(f' env cur ', env.pos)
            ...
            for _ in range(config.EXPLORE_HORIZON):
                ...
                for _ in range(config.STEPS_PER_ACTION):
                    if current_batch_steps_counter >= STEPS_LIMIT_PER_BATCH: break
                    rpm, _ = controller.compute_action(env.CTRL_FREQ, env._computeMapObs(), target_vel)
                    obs, reward, terminated, _, _ = env.step(rpm)

                    # === [New] Height Constraint Check ===
                    # If drone flies higher than ceiling, force termination
                    if env.pos[0][2] > CEILING_HEIGHT:
                        terminated = True
                    # =====================================
    ...
    for elite_node in candidates:
        if elite_node.cumulative_reward > score_threshold:
            ...
            print(f'final_reward {traj_info["final_reward"]}')
...
def main():
    ...
    BUDGET = 50000
    REPEATS = 3
    SCORE_THRESHOLD = 98.0
    # Save file with a new name to reflect the ceiling constraint
    SAVE_FILE = "sge_full_state_trajectories_with_obs20_2.pkl"

    all_data = {}
    total_count = 0

    print("=== STARTING FULL STATE EXTRACTION (WITH CEILING) ===")
    print(f"Ceiling Height Limit: {CEILING_HEIGHT}m")
    ...
    print("Height constraint applied.")
